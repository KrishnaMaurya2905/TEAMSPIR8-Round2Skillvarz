...
//           className="fixed max-md:top-[90%] max-md:w-[98%] max-md:h-[13vh] top-1/2 -translate-y-1/2 right-5 z-[3] flex w-[8%] h-[96vh]  will-change-transform md:flex-col gap-3 md:justify-start pointer-events-auto overflow-scroll max-xl:w-[15%] max-2xl:right-3 max-2xl:w-[10%] max-md:p-1 max-md:right-0"
//         >
//           {data.map((item, index) => {
...
//                 onClick={(e) => handleDivClick(e, item, index, navigate)}
...
// const Sidebar = ({ current, setCursorLabel, sidebarRef }) => {
...
const Sidebar = ({ current, setCursorLabel }) => {
  ...
  // Handle custom smooth scroll for non-mobile
  useEffect(() => {
    const scrollToElementWithEasing = (container, target) => {
      const start = container.scrollTop;
      const end = target.offsetTop - container.offsetTop;
  ...
  // Stop scroll events from bubbling out of the sidebar
  useEffect(() => {
    ...
    <AnimatePresence>
      {showSidebar && (
        <motion.div
          ...
          exit={{ y: "100vh", opacity: 0 }}
          transition={{ duration: 1, ease: [0.23, 1, 0.32, 1] }}
          className="sidebar-container fixed max-md:top-[90%] max-md:w-[98%] max-md:h-[13vh] top-1/2 -translate-y-1/2 right-5 z-[3] flex w-[8%] h-[96vh] md:flex-col gap-3 md:justify-start pointer-events-auto overflow-auto xl:overflow-scroll max-xl:w-[15%] max-2xl:right-3 max-2xl:w-[10%] max-md:p-1 max-md:right-0"
          style={{ willChange: "transform" }}
        >
          {data.map((item, index) => {
            const isActive = index === current;
